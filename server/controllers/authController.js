...
const generateToken = (id) => {
  ...
  if (!secret || secret === "") {
    console.warn(
      "⚠️  WARNING: JWT_SECRET is not set. Using default secret. This is insecure for production!"
    );
  }
...
const register = async (req, res, next) => {
  try {
    ...
    // Clear stats cache when new user is created
    clearStatsCache();
...
const login = async (req, res, next) => {
  try {
    ...
    if (user.matchPassword) {
      ...
      if (!user.password) {
        console.log(
          `Login attempt failed: User ${user.email} has no password stored`
        );
        return res.status(401).json({ message: "Invalid credentials" });
      }
